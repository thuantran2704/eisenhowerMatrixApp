...
class EisenhowerMatrixApp:
    ...
    def add_task(self, event):
        ...
        if task_label:
            x, y = event.x, event.y
            task_rect = self.canvas.create_rectangle(x - 50, y - 20, x + 50, y + 20, fill="lightblue", outline="black")
            ...
            self.canvas.create_window(x + 40, y - 20, window=button)  # Position the button

            # Store task and button references in dictionary
            self.tasks[task_rect] = {"text": task_text, "x": x, "y": y, "description": task_label, "button": button}
    ...
    def move_task(self, event):
        ...
        if self.dragging_task:
            ...
            self.canvas.coords(task_rect, x - 50, y - 20, x + 50, y + 20)
        ...
        try:
            with open("tasks.json", "r") as file:
                ...
                for task in tasks_data:
                    ...
                    # Store task and button references in dictionary
                    self.tasks[task_rect] = {"text": task_text, "x": x, "y": y, "description": description, "button": button}
